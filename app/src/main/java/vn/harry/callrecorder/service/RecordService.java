...
public class RecordService extends Service {
    ...
    private void stopAndReleaseRecorder() {
        ...
        if (recorderStopped) {
            ...
            if (RecordingUtils.clientListSize(getBaseContext()) > 0) {
                boolean isClientMatched = false;
                for (int i = 0; i < RecordingUtils.getClientsList(getBaseContext()).size(); i++) {
                    if (RecordingUtils.getClientsList(getBaseContext()).get(i).getMobilenumber().equals(Recording.getAllItems().get(i).phoneNumber)) {
                        //Upload Recording
                        recording.setIdCall(idCall);
                        recording.setOutGoing(App.isOutComming);
                        App.isOutComming = false;
                        ...
                        file = null;
                        Log.d(Constants.TAG, "RecordService save");
                        isClientMatched = true;
                        break;
                    } else {
                        isClientMatched = false;
                    }
                }
                if (!isClientMatched) {
                    deleteFile();
